...
function openMiniModal(item){
  ...
  const tg = (Array.isArray(item.tags)&&item.tags[0]) ? item.tags[0] : "Offline";
  const cls = (typeof TAG_COLOR!=="undefined" && TAG_COLOR[tg]) ? TAG_COLOR[tg] : "offline";
  const chip = document.createElement("span"); chip.className = `tag-pill tag-${cls==="+"+"18" ? "18" : cls}`;
  chip.textContent = `${(typeof TAG_EMOJI!=="undefined" && TAG_EMOJI[tg])?TAG_EMOJI[tg]:""} ${(typeof TAG_LABEL!=="undefined" && TAG_LABEL[tg])?TAG_LABEL[tg]:tg}`.trim();
  const sep = document.createElement("span"); sep.className = "mini-bullet"; sep.textContent = "•";
  const vwrap = document.createElement("span"); vwrap.className="mini-version";
  const vlabel = document.createElement("span"); vlabel.className="mini-version-label"; vlabel.textContent = "Versión:";
  const vval = document.createElement("span"); vval.className="mini-version-value"; vval.textContent = safe(item.version, "v1.0");
  vwrap.appendChild(vlabel); vwrap.appendChild(document.createTextNode(" ")); vwrap.appendChild(vval);
  meta.appendChild(chip); meta.appendChild(sep); meta.appendChild(vwrap);
